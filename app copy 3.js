...
app.post('/webhook', (req, res) => {
    ...
    if (messages.type === 'interactive' && messages.interactive.type === 'button_reply') {
        ...
        if (buttonId === 'optionBangla') {
            ...
            const lang = userLanguagePreference[from] || 'english';
            sendBtrcHelplineCallButton(from, lang);
            // const reply = (lang === 'bangla')
...
function sendBtrcHelplineCallButton(to, language = 'english') {
    const params = {
        messaging_product: "whatsapp",
        to,
        type: "interactive",
        interactive: {
            ...
            body: {
                text: (language === 'bangla') 
                    ? "বিটিআরসি হেল্পলাইনে সরাসরি কল করতে নিচের বোতামে ক্লিক করুন।" 
                    : "Click the button below to call BTRC Helpline directly."
            },
            action: {
                buttons: [
                    {
